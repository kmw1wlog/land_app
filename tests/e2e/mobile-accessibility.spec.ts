...
test("discovery feed shows real-transaction candidate affordances", async ({ page }) => {
  ...
  await expect(page.getByText("내 구매력 요약")).toBeVisible();
  await expect(page.getByText("공공 실거래가 기반 분석 후보").first()).toBeVisible();
  await expect(page.getByText("최근 실거래 기준가")).toBeVisible();
  await expect(page.getByText("주거 이동 체크리스트")).toBeVisible();
  await expect(page.getByText("층별 실거래 기준가")).toBeVisible();
  await expect(page.getByRole("link", { name: "외부 사이트에서 매물 확인" }).last()).toBeVisible();
  await page.getByRole("link", { name: /같은 돈 비교/ }).click();
  await expect(page.getByText("같은 예산이면 어디가 더 안전할까?")).toBeVisible();
  await page.goto("/feed");
  await page.getByRole("link", { name: /도달경로/ }).first().click();
  await expect(page.getByRole("heading", { name: "내 주거 경로" }).first()).toBeVisible();
  await expect(page.getByText("가격대에 접근하는 경로")).toBeVisible();
});

test("submission demo pages render guided recording and capture flows", async ({ page }) => {
  await page.setViewportSize({ width: 1920, height: 1080 });
  await page.goto("/demo-submission");
  await expect(page.getByRole("heading", { name: /지금 내 조건으로/ })).toBeVisible();
  await page.getByRole("button", { name: "다음" }).click();
  await expect(page.getByRole("heading", { name: "내 주거 조건을 입력합니다" })).toBeVisible();
