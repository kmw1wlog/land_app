...
const requiredColumns = ["month", "region", "lawdCode5", "guaranteeAccidentRate", "jeonseRiskScore", "riskGrade"];
...
async function main() {
  const source = process.env.HUG_SOURCE_URL;
  if (!source) {
    console.log(JSON.stringify({ skipped: true, reason: "HUG_SOURCE_URL is empty" }, null, 2));
    return;
  }
  const csv = await loadSource(source);
  const normalized = normalizeCsv(csv, source);
  mkdirSync(path.join(process.cwd(), "data/fusion/raw/hug"), { recursive: true });
  writeFileSync(path.join(process.cwd(), "data/fusion/raw/hug/hug_jeonse_risk_raw.csv"), csv);
  writeFileSync(path.join(process.cwd(), "data/fusion/hug_jeonse_risk_real.csv"), normalized);
  console.log(JSON.stringify({ skipped: false, outputFile: "data/fusion/hug_jeonse_risk_real.csv", sourceUrl: source }, null, 2));
}
